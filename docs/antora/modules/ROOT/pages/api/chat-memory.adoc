...
| `collection` | `Collection` | `null` | Pre-configured MongoDB collection to use directly.
| `db` | `Db` | `null` | MongoDB database used to resolve the collection by name.
| `mongoClient` | `MongoClient` | `null` | MongoDB client used to resolve the collection by name.
| `collectionName` | `string` | `ai_chat_memory` | Name of the collection used when resolving from `db` or `mongoClient`.
| `createIndices` | `boolean` | `false` | When `true`, creates the chat memory indexes on startup.
| `ttl` | `number` | `null` | When set, creates or updates the TTL index on the `timestamp` field.
|===
