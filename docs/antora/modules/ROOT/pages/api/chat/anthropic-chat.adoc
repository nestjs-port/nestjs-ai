...
| topP | Nucleus sampling parameter. The model considers tokens with top_p probability mass. Supported on Claude Opus 4.6 and earlier models only. Deprecated by Anthropic for models released after Claude Opus 4.6 (for example `claude-opus-4-7`); values `< 0.99` are rejected with HTTP 400, and values `>= 0.99` are accepted as a backward-compatibility no-op. | -
| topK | Only sample from the top K options for each token. Supported on Claude Opus 4.6 and earlier models only. Deprecated by Anthropic for models released after Claude Opus 4.6 (for example `claude-opus-4-7`); any value is rejected with HTTP 400 on those models. | -
