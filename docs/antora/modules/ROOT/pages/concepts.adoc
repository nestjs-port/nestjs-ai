...
This section describes core concepts that NestJS AI uses. We recommend reading it closely to understand the ideas behind how NestJS AI is implemented.
...
NestJS AI currently supports models that process input and output as language, image, and audio.
...
NestJS AI has support for embeddings to enable more advanced use cases.
...
NestJS AI employs the OSS library https://www.stringtemplate.org/[StringTemplate] for this purpose.
...
In NestJS AI, prompt templates can be likened to the "View" layer of a web application.
A model object, typically a `Record<string, unknown>` (or `Map<string, unknown>`), is provided to populate placeholders within the template.
...
As a TypeScript developer exploring AI, it's not necessary to comprehend the intricate mathematical theories or the specific implementations behind these vector representations.
...
The NestJS AI project helps you with this task.

== Structured Output

The output of AI models traditionally arrives as a `string`, even if you ask for the reply to be in JSON.
...
The NestJS AI library helps you implement solutions based on the "`stuffing the prompt`" technique otherwise known as xref::concepts.adoc#concept-rag[Retrieval Augmented Generation (RAG)].
...
NestJS AI greatly simplifies code you need to write to support xref:api/tools.adoc[tool calling].
...
image::spring-ai-rag.jpg[NestJS AI RAG, width=1000, align="center"]
...
NestJS AI greatly simplifies code you need to write to support tool invocation.
It handles the tool invocation conversation for you.
You can provide your tool as a `@Tool`-decorated method and provide it in your prompt options to make it available to the model.
...
The NestJS AI project provides an `Evaluator` API which currently gives access to basic strategies to evaluate model responses.
