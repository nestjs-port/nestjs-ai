...
NOTE: NestJS AI targets NestJS 10+ or 11+ (peer dependency range `^10.0.0 || ^11.0.0`) and requires Node.js 20+.
...
pnpm add @nestjs-ai/platform @nestjs-ai/model-openai @nestjs-ai/client-chat \
  @nestjs-ai/commons @nestjs-ai/model @nestjs-ai/template-st \
...
Inject `CHAT_MODEL_TOKEN` and create a `ChatClient` with `ChatClient.create(chatModel)`, or
