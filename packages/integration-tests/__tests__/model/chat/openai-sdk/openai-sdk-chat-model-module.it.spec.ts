...
describe.skipIf(!OPENAI_API_KEY)("OpenAiSdkChatModelModuleIT", () => {
  const logger = LoggerFactory.getLogger("OpenAiSdkChatModelModuleIT");
  ...
  it("generate streaming", async () => {
    ...
    const response = await collectChatResponseText(
      chatModel.stream(
        new Prompt("Hello", OpenAiSdkChatOptions.builder().build()),
      ),
    );
  ...
  it("chat activation", async () => {
    ...
    expect(defaultOptions.model).toBe(OpenAiSdkChatOptions.DEFAULT_CHAT_MODEL);
...
async function createChatModel(
  ...
  const moduleRef = await Test.createTestingModule({
    imports: [OpenAiSdkChatModelModule.forFeature(properties)],
  }).compile();
