...
describe("OpenAiSdkChatModelModule", () => {
  describe("forFeature", () => {
    it("should resolve CHAT_MODEL_TOKEN via NestJS DI", async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkChatModelModule.forFeature({
            apiKey: "test-key",
          }),
        ],
      }).compile();
    ...
    it("should fall back to the default chat model", async () => {
      ...
      expect(chatModel.options.model).toBe(OPEN_AI_SDK_CHAT_DEFAULT_MODEL);
    });

    it("should not export the properties token", async () => {
      const featureModule = OpenAiSdkChatModelModule.forFeature({
        apiKey: "test-key",
      });
      ...
      expect(exports).not.toContain(OPEN_AI_SDK_CHAT_PROPERTIES_TOKEN);
    });
  ...
  describe("forFeatureAsync", () => {
    it("should resolve CHAT_MODEL_TOKEN from async factory via NestJS DI", async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkChatModelModule.forFeatureAsync({
            useFactory: () => ({
      ...
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkChatModelModule.forFeatureAsync({
            ...
            useFactory: (apiKey: string): OpenAiSdkChatProperties => ({
              apiKey,
              model: "gpt-5-pro",
              options: {
                user: "async-user",
