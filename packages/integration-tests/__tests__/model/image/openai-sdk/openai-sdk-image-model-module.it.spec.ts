...
const logger = LoggerFactory.getLogger("OpenAiSdkImageModelModuleIT");

async function createImageModel(
  properties: OpenAiSdkImageProperties,
): Promise<OpenAiSdkImageModel> {
  const moduleRef = await Test.createTestingModule({
    imports: [OpenAiSdkImageModelModule.forFeature(properties)],
  }).compile();

  return moduleRef.get<OpenAiSdkImageModel>(IMAGE_MODEL_TOKEN);
}

describe.skipIf(!OPENAI_API_KEY)("OpenAiSdkImageModelModuleIT", () => {
  ...
  it("image activation", async () => {
    ...
    expect(defaultImageModel.options.model).toBe(
      OPEN_AI_SDK_IMAGE_DEFAULT_MODEL,
    );
