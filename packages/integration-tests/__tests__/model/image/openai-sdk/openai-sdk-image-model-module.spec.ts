...
describe("OpenAiSdkImageModelModule", () => {
  describe("forFeature", () => {
    it("should resolve IMAGE_MODEL_TOKEN via NestJS DI", async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkImageModelModule.forFeature({
            apiKey: "test-key",
          }),
        ],
      }).compile();
    ...
    it("should fall back to the default image model", async () => {
      ...
      expect(imageModel.options.model).toBe(OPEN_AI_SDK_IMAGE_DEFAULT_MODEL);
    });

    it("should not export the properties token", async () => {
      const featureModule = OpenAiSdkImageModelModule.forFeature({
        apiKey: "test-key",
      });
      ...
      expect(exports).not.toContain(OPEN_AI_SDK_IMAGE_PROPERTIES_TOKEN);
    });
  ...
  describe("forFeatureAsync", () => {
    it("should resolve IMAGE_MODEL_TOKEN from async factory via NestJS DI", async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkImageModelModule.forFeatureAsync({
            useFactory: () => ({
      ...
      const moduleRef = await Test.createTestingModule({
        imports: [
          OpenAiSdkImageModelModule.forFeatureAsync({
            ...
            useFactory: (apiKey: string): OpenAiSdkImageProperties => ({
              apiKey,
              model: "gpt-image-1",
              options: {
                user: "async-user",
