...
export class McpResourceListChangedConsumerMethodException extends Error {
  constructor(message: string, options?: { cause?: unknown }) {
    super(message, options);
    this.name = "McpResourceListChangedConsumerMethodException";
  }
}
 ...
 */
export abstract class AbstractMcpResourceListChangedMethodCallback {
  ...
  protected validateParameters(): void {
    ...
    if (paramTypes.length !== 1) {
      throw new Error(
        `Method must have exactly 1 parameter (List<McpSchema.Resource>): ${this.methodName} in ${this.declaringClassName} has ${paramTypes.length} parameters`,
      );
    }
    ...
    if (paramType !== Array) {
      throw new Error(
        `Parameter must be of type List<McpSchema.Resource>: ${this.methodName} in ${this.declaringClassName} has parameter of type ${paramType?.name ?? "unknown"}`,
      );
    }
  }
