...
describe.skipIf(!OPENAI_API_KEY)("OpenAiSdkChatModel IT", () => {
  const logger = LoggerFactory.getLogger("OpenAiSdkChatModelIT");

  const systemPromptResource = readFileSync(
    resolve(__dirname, "system-message.st"),
  );

  const chatModel = new OpenAiSdkChatModel({
  ...
  it("streaming with token usage", async () => {
    const promptOptions = OpenAiSdkChatOptions.builder()
  ...
  it("multi modality embedded image", async () => {
    ...
    const response = await chatModel.call(
      new Prompt([userMessage], OpenAiSdkChatOptions.builder().build()),
    );
  ...
  it("validate call response metadata", async () => {
    const model = OpenAiSdkChatOptions.DEFAULT_CHAT_MODEL;
  ...
  it("chat memory with tools", async () => {
    ...
    const chatOptions = OpenAiSdkChatOptions.builder()
      .toolCallbacks([
        FunctionToolCallback.builder(
          "multiply",
