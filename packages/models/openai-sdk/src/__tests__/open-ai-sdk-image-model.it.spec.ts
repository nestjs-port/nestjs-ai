...
import { OpenAiSdkImageGenerationMetadata } from "../metadata";
...
describe.skipIf(!OPENAI_API_KEY)("OpenAiSdkImageModelIT", () => {
  const imageModel = new OpenAiSdkImageModel({
  ...
  it("image as url test", async () => {
    ...
    const openAiSdkImageGenerationMetadata =
      imageGenerationMetadata as OpenAiSdkImageGenerationMetadata;

    expect(openAiSdkImageGenerationMetadata).not.toBeNull();
    expect(openAiSdkImageGenerationMetadata.revisedPrompt?.trim()).toBeTruthy();
  });
});
