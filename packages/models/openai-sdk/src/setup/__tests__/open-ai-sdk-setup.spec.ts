...
import { ModelProvider, OpenAiSdkSetup } from "../open-ai-sdk-setup";

describe("OpenAiSdkSetupTests", () => {
  it("detect model provider returns microsoft foundry when microsoft foundry flag is true", () => {
    const result = OpenAiSdkSetup.detectModelProvider(
      true,
      false,
      null,
      null,
  ...
  it("setup sync client returns client when valid api key provided", () => {
    const client = OpenAiSdkSetup.setupClient({
  ...
  it("calculate base url returns default open ai url when base url is null", () => {
    const result = OpenAiSdkSetup.calculateBaseUrl(
      null,
    ...
    expect(result).toBe(OpenAiSdkSetup.OPENAI_URL);
  });
  ...
  it("calculate base url returns github url when model host is github", () => {
    ...
    expect(result).toBe(OpenAiSdkSetup.GITHUB_MODELS_URL);
  });
