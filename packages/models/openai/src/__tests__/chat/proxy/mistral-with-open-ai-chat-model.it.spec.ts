...
const MISTRAL_AI_API_KEY = process.env.MISTRAL_AI_API_KEY;

const MISTRAL_BASE_URL = "https://api.mistral.ai";

const MISTRAL_DEFAULT_MODEL = "mistral-small-latest";

describe.skipIf(!MISTRAL_AI_API_KEY)("MistralWithOpenAiChatModelIT", () => {
  ...
  it("stream role test", async () => {
    ...
    const prompt = new Prompt([userMessage, systemMessage]);
    const flux = chatModel.stream(prompt);

    const responses = await lastValueFrom(flux.pipe(toArray()));
