...
describe.skipIf(!PERPLEXITY_API_KEY)("PerplexityWithOpenAiChatModelIT", () => {
  ...
  it("streaming with token usage", async () => {
    ...
    expect(streamingTokenUsage.completionTokens).toBeGreaterThanOrEqual(
      referenceTokenUsage.completionTokens,
    );
    expect(streamingTokenUsage.totalTokens).toBeGreaterThanOrEqual(
  ...
  it("map output converter", async () => {
    ...
    const promptTemplate = PromptTemplate.builder()
      .template(template)
      .variables({
        subject: "numbers from 1 to 9 under the key name 'numbers'",
        format,
      })
      .build();
    const prompt = new Prompt(promptTemplate.createMessage());
  ...
  it("validate call response metadata", async () => {
    ...
    expect(response.metadata.model.toLowerCase()).toContain(
      DEFAULT_PERPLEXITY_MODEL.toLowerCase(),
    );
