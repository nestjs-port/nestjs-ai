...
import { LoggerFactory, type Milliseconds, ms } from "@nestjs-port/core";
import type { ClientOptions } from "openai";
import OpenAI, { AzureOpenAI } from "openai";

export interface OpenAiSetupProps {
  baseUrl?: string | null;
  apiKey?: string | null;
...
export class OpenAiSetup {
  ...
  static readonly DEFAULT_USER_AGENT = "spring-ai-openai";

  private static readonly DEFAULT_DURATION: Milliseconds = ms(60_000);
  private static readonly DEFAULT_MAX_RETRIES = 3;
  private static readonly logger = LoggerFactory.getLogger(OpenAiSetup.name);

  private constructor() {}

  static setupClient(props: OpenAiSetupProps): OpenAI | AzureOpenAI {
    const baseUrl = OpenAiSetup.detectBaseUrlFromEnv(props.baseUrl ?? null);
    const azureOpenAiServiceVersion =
      typeof props.azureOpenAiServiceVersion === "string"
        ? props.azureOpenAiServiceVersion
        : null;
    const modelProvider = OpenAiSetup.detectModelProvider(
      props.isAzure ?? false,
    ...
    const timeout = props.timeout ?? OpenAiSetup.DEFAULT_DURATION;
    const maxRetries = props.maxRetries ?? OpenAiSetup.DEFAULT_MAX_RETRIES;
    const calculatedBaseUrl = OpenAiSetup.calculateBaseUrl(
      baseUrl,
      modelProvider,
    ...
    const calculatedApiKey =
      props.apiKey ?? OpenAiSetup.detectApiKey(modelProvider) ?? undefined;
    const authProvider =
      props.azureADTokenProvider ?? OpenAiSetup.azureAuthentication();
    const azureADTokenProvider =
      calculatedApiKey == null ? authProvider : undefined;
    const defaultHeaders = {
      "User-Agent": OpenAiSetup.DEFAULT_USER_AGENT,
  ...
  static detectModelProvider(
    ...
    if (baseUrl != null) {
      ...
      if (baseUrl.startsWith(OpenAiSetup.GITHUB_MODELS_URL)) {
  ...
  static calculateBaseUrl(
    ...
    if (modelProvider === ModelProvider.OPEN_AI) {
      return baseUrl == null || baseUrl.trim().length === 0
        ? OpenAiSetup.OPENAI_URL
        : baseUrl;
    }

    if (modelProvider === ModelProvider.GITHUB_MODELS) {
      if (baseUrl == null || baseUrl.trim().length === 0) {
        return OpenAiSetup.GITHUB_MODELS_URL;
      }
      if (baseUrl.startsWith(OpenAiSetup.GITHUB_MODELS_URL)) {
  ...
  static detectApiKey(modelProvider: ModelProvider): string | null {
    if (
      ...
      process.env[OpenAiSetup.OPENAI_API_KEY] != null
    ) {
      return process.env[OpenAiSetup.OPENAI_API_KEY] ?? null;
    }

    if (
      modelProvider === ModelProvider.MICROSOFT_FOUNDRY &&
      process.env[OpenAiSetup.MICROSOFT_FOUNDRY_API_KEY] != null
    ) {
      return process.env[OpenAiSetup.MICROSOFT_FOUNDRY_API_KEY] ?? null;
    }
    ...
    if (
      ...
      process.env[OpenAiSetup.GITHUB_TOKEN] != null
    ) {
      return process.env[OpenAiSetup.GITHUB_TOKEN] ?? null;
    }
